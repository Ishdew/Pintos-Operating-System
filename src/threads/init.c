...
pintos_init (void)
{
  ...
  if (*argv != NULL) {
    ...
    while (true)
    {
      ...
      char buffer[256];
      int place = 0;
      bool loop_controller = false; // Loop controller

      while (!loop_controller )
      {
        char placeHolder = input_getc();
        if (placeHolder == '\n' || placeHolder == '\r')
        {
          buffer[place] = '\0';
      ...
      char *saver;
      char *tokenizer = strtok_r(buffer, " ", &saver);
      if (tokenizer != NULL)
      {
        if (strcmp(tokenizer, "whoami") == 0) // strcmp returns 0 if the strings are equal
        {
          printf("Dewpura L.C.I.K - 210116A\n");
        }
        else if (strcmp(tokenizer, "shutdown") == 0)
        {
          printf("Shutting down. Bye !!! \n");
          shutdown_power_off();
        }
        else if (strcmp(tokenizer, "time") == 0)
        {
          time_t current_time = (time_t)timer_ticks() / TIMER_FREQ;
          printf("seconds: %lld seconds\n", (long long)current_time);
        }
        else if (strcmp(tokenizer, "ram") == 0)
        {
          size_t ram_total = init_ram_pages * PGSIZE / 1024;   
          printf ("Pintos storage  %'"PRIu32" kB RAM...\n", ram_total);  // init_ram_pages is defined in threads/init.h
        }
        else if (strcmp(tokenizer, "thread") == 0)
        {
          thread_print_stats();
        }
        else if (strcmp(tokenizer, "priority") == 0)
        {
          int current_priority = thread_get_priority();
          printf("Priority: %d\n", current_priority);
        }
        else if (strcmp(tokenizer, "exit") == 0)
        {
